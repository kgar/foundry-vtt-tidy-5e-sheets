...
<aside
  class="sidebar"
  style:--t5e-member-color-hover={`oklch(from ${emphasizedMember?.accentColor} calc(l * 1.4) 60% h)`}
  style:--t5e-member-color-background={`oklch(from ${emphasizedMember?.accentColor} calc(l * 0.75) calc(c * 1.2) h)`}
>
  <!-- Aggregate Traits -->
