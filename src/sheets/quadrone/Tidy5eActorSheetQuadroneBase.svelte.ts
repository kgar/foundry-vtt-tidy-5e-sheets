  ...
  abstract class Tidy5eActorSheetQuadroneBase extends getTidyExtensibleDocumentSheetMixin(
    ...
    async _preparePortrait(
      ...
      return {
        ...
        shape: showToken
          ? 'token'
          : (themeSettings.portraitShape ??
            ThemeQuadrone.DEFAULT_PORTRAIT_SHAPE),
        isVideo,
        isRandom,
      };
    }
    ...
    _prepareTraits(): Record<string, ActorTraitContext[]> {
      ...
      for (const key of this.actor.system.traits?.weaponProf?.mastery?.value ??
        ...
        (value.icons ??= []).push({
          icon: 'fa-solid fa-circle-star mastery',
    ...
    _getCreatureType(): CreatureTypeContext {
      ...
      return {
        icon: creatureType?.icon ?? 'icons/svg/mystery-man.svg',
        title:
    ...
    _onDragStart(
      ...
      if (
        el.matches('[data-item-id] > [data-tidy-table-row], [data-item-id]')
      ) {
    ...
    _onDragJournal(
      ...
      const journalSortDragData = {
        ...
        uuid: event.target.closest<HTMLElement>('[data-document-uuid]')?.dataset
          .documentUuid,
    ...
    static async #roll(
      ...
      if (this._roll(event, target) === false || !this.isEditable) {
        return;
      }
          ...
          return this.actor.rollDeathSave(
            { event, legacy: false },
            {
              options: {
                default: {
                  rollMode:
                    SettingsProvider.settings.defaultDeathSaveRoll.get(),
                },
              },
            },
          );
