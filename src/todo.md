...
- [ ] Redesign the _onChangeForm handling. _onChangeForm is not allowed to be async. Use an alternate function that can be called by _onChangeForm, but the method itself should not be async, since it's too easy to break off currentTarget with an async operation.
