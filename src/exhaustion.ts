// TODO: Make this work.
/*
    // ALSO, there is this code to call when someone actively changes exhaustion on an actor, to be triggered at the time when the user clicks the LI for an exhaust level:
...
// import type { Actor5e } from './foundry/foundry-adapter.js';
// import { debug, warn } from './utils/logging.js';

// export async function updateExhaustion(actorEntity: Actor5e) {
//   const actorC = game.actors.get(actorEntity._id);
//   if (!actorC) {
//     warn(
//       `The actor with id '${actorEntity._id}' must exists for apply a Exhaustion`
//     );
//     return;
//   }
//   if (actorC.type !== 'character' && actorC.type !== 'npc') {
//     warn(
//       `The actor with id '${actorEntity._id}' must be a Character or a NPC for apply a Exhaustion`
//     );
//     return;
//   }
...
//   if (
//     game.settings.get(CONSTANTS.MODULE_ID, 'exhaustionEffectsEnabled') ==
//     'tidy5e'
//   ) {
//     let icon = game.settings.get(CONSTANTS.MODULE_ID, 'exhaustionEffectIcon');
//     let currentExhaustion;
//     let exhaustionPresent = null;
//     let effectName = `${game.i18n.localize(
//       'DND5E.ConExhaustion'
//     )} ${game.i18n.localize('DND5E.AbbreviationLevel')} ${exhaustion}`;
...
//           movementSet = [];
//           Object.entries(actorEntity.system.attributes.movement).forEach(
//             (speed) => {
...
//       if (
//         typeof effectEntity.getFlag(CONSTANTS.MODULE_ID, 'exhaustion') ===
//         'number'
//       ) {
//         exhaustionPresent = effectEntity;
//         currentExhaustion = effectEntity.getFlag(
//           CONSTANTS.MODULE_ID,
//           'exhaustion'
//         );
//         debug(
//           `tidy5e-exhaustion | updateExhaustion | currentExhaustion: ${currentExhaustion}`
//         );
...
//       if (exhaustion > 0) {
//         debug(
//           `tidy5e-exhaustion | createExhaustionEffect | create Effect exhaustion lv: ${exhaustion}`
//         );
//         let effectChange = {
//           disabled: false,
//           label: effectName,
...
//         await actorEntity.createEmbeddedDocuments('ActiveEffect', [
//           effectChange,
//         ]);
...
//       const levels = game.settings.get(
//         CONSTANTS.MODULE_ID,
//         'exhaustionEffectCustomTiers'
//       );
//       const effectName = game.settings.get(
//         CONSTANTS.MODULE_ID,
...
//           if (
//             game.dfreds.effectInterface.hasEffectApplied(
//               tier,
//               actorToCheck.uuid
//             )
//           ) {
...
//       if (exhaustion != 0) {
//         let effectToCheck =
//           game.dfreds.effectInterface.findEffectByName(effectNameCustom);
//         if (!effectToCheck) {
...
//     if (
//       effectName.includes(
//         game.settings.get(CONSTANTS.MODULE_ID, 'exhaustionEffectCustom')
//       )
//     ) {
//       debug(
//         'tidy5e-exhaustion | createActiveEffect | effectName = ' + effectName
//       );
//       if (actor.type === 'character') {
...
//       } else {
//         warn(
//           `createActiveEffect is not supported for actor ype '${actor.type}'`
//         );
//       }
//     }
//   }
...
//       const actorEffects =
//         (token && token.actor ? token.actor?.effects : actor?.effects) || [];
...
//       } else {
//         warn(
//           `deleteActiveEffect is not supported for actor ype '${actor.type}'`
//         );
//       }
//     }
//   }
