...
export default class EnchantmentConfig extends DocumentSheet {
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize('DND5E.Enchantment.Configuration')}: ${
      this.document.name
    }`;
  }
  ...
  async getData(options = {}) {
    ...
    context.enchantableTypes =
      dnd5e.dataModels.item.EnchantmentData.enchantableTypes.reduce(
        (obj, k) => {
    ...
    context.enchantments = context.enchantments.map((effect) => ({
      ...
      riderEffects: effects.map(({ id, name }) => ({
        id,
        name,
        selected: effect.flags.dnd5e?.enchantment?.riders?.effect?.includes(id)
          ? 'selected'
          : '',
      })),
      riderItems:
        effect.flags.dnd5e?.enchantment?.riders?.item?.join(',') ?? '',
    }));
  ...
  activateListeners(jQuery) {
      ...
      element.addEventListener('click', (event) =>
        this.submit({
          updateData: {
            ...
            enchantmentId: event.target.closest('[data-enchantment-id]')
              ?.dataset.enchantmentId,
          },
        })
      );
    }
    ...
    for (const element of html.querySelectorAll('.collapsible')) {
      element.addEventListener('click', (event) => {
        const id = event.target.closest('[data-enchantment-id]')?.dataset
          .enchantmentId;
  ...
  async _updateObject(event, formData) {
      ...
      ([_id, changes]) => {
        ...
        if (
          !foundry.utils.hasProperty(
            updates,
            'flags.dnd5e.enchantment.riders.effect'
          )
        ) {
    ...
    for (const effect of this.document.effects) {
      ...
      if (riderIds.has(effect.id))
        effectsChanges.push({ _id: effect.id, 'flags.dnd5e.rider': true });
      else effectsChanges.push({ _id: effect.id, 'flags.dnd5e.-=rider': null });
    }
    if (effectsChanges.length)
